...
- **3. Odds for Different Market Types** ([pages/market-types-overview.html](pages/market-types-overview.html)) — the given matrix every market in this section starts from, settlement logic (which scorelines make a bet win, and which don't settle cleanly), and the shared odds conversion chain.
  ...
  - **3.4 Total Goals** ([pages/total-goal-market.html](pages/total-goal-market.html)) — the matrix cells become click targets across four fixed buckets from `bookie_example.png` (0~1 / 2~3 / 4~6 / 7 & Over), then Check verifies the whole grid and unlocks a Total Goals Fair Odds table (Probability → Euro Odds only - no HK/Malay for this market).
