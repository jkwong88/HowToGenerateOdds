...
- **3. Odds for Different Market Types** ([pages/market-types-overview.html](pages/market-types-overview.html)) — the given matrix every market in this section starts from, settlement logic (which scorelines make a bet win, and which don't settle cleanly), and the shared odds conversion chain.
  - **3.1 Correct Score** ([pages/correct-score-odds.html](pages/correct-score-odds.html)) — carries the matrix over as given data, then builds a Correct Score board (0:0 … 4:4, plus AOS) split into three sub-tables by Home value. Only the Away = 0 column is the exercise; passing it reveals the rest. Each scoreline converts Fair Probability → Euro Odds → HK Odds → Malay Odds.
  - **3.2 Odd / Even** ([pages/odd-even-market.html](pages/odd-even-market.html)) — a category picker (Odd / Even) lets the student choose which category they're painting and switch or repaint any cell at any time; Check Classification then verifies the whole grid at once (right/wrong cells get a green/red background plus a ✓/✗ badge, so feedback isn't color-only). Passing unlocks an Odd/Even Fair Odds table (Fair Probability → Euro Odds → HK Odds → Malay Odds).
  - **3.3 1X2 & Double Chance** ([pages/1x2-double-chance-market.html](pages/1x2-double-chance-market.html)) — Part 1 (1X2): a category picker (Home / Draw / Away) works the same way as 3.2's, then Check Classification verifies the grid and unlocks a 1X2 Fair Odds table. Only Probability is graded there; Euro Odds is a derived value that reveals once Probability checks out, same as Double Chance's own Euro Odds column below it. Passing that table's Check unlocks Part 2 (Double Chance): fill in the Probability for 1X/12/X2 by summing the matching pair of 1X2 probabilities; Euro Odds again reveals once Probability checks out (see [ADR 0001](docs/adr/0001-merge-1x2-and-double-chance.md)).
  - **3.4 Total Goals** ([pages/total-goal-market.html](pages/total-goal-market.html)) — a category picker across four fixed buckets from `bookie_example.png` (0–1 / 2–3 / 4–6 / 7+), then Check Classification verifies the whole grid and unlocks a Total Goals Fair Odds table. Only Probability is graded there; Euro Odds is a derived value that reveals once Probability checks out (no HK/Malay for this market) - odds conversion isn't this exercise's focus, since 3.1-3.3 already drill it.
  - **3.5 Over / Under** ([pages/over-under-market.html](pages/over-under-market.html)) — a Line dropdown (2, 2.25, 2.5, 2.75, default 2.5) changes the classification: a whole-number line has 3 categories (Under/Push/Over, a real refundable push), a `.25`/`.75` line also has 3 (Under/Split Settlement/Over, per the reference slides' EV derivation - the pivot scoreline is a Half Loss for whichever side sits above it and a Half Win for the other), and a `.5` line has only 2 (Under/Over, no push possible); the category picker rebuilds to match whichever set applies. The raw category table (Raw Outcome Probability, 2 decimals) feeds a separate Selection conversion table (Settlement-Adjusted Fair Probability, also 2 decimals - only Under/Over, since a push or split settlement isn't itself bettable), which is always graded regardless of line. Euro/HK/Malay Odds are only graded at the default line 2.5 (`DEFAULT_POINT` in `js/over-under-market.js`) - that conversion is already drilled in 3.1-3.4, so every other line shows it as a derived `.given-value` instead of re-testing it.
  - **3.6 HDP** ([pages/hdp-market.html](pages/hdp-market.html)) — a Line dropdown (-1 to +1 in quarter-point steps, standard Asian Handicap sign, default Home +1) reuses Over/Under's exact point-type math, precision policy, and default-line-only odds grading (`DEFAULT_LINE` in `js/hdp-market.js`), just classifying Home − Away instead of Home + Away (see [ADR 0002](docs/adr/0002-generalize-point-type-math-for-hdp.md)). A category picker (Away Covers / Push-or-Split / Home Covers) works the same way as 3.2's, then Check Classification unlocks an HDP Fair Odds table.
- **4. Opening Market** ([pages/opening-market.html](pages/opening-market.html)) — explains what "opening" a market means (choosing a specific line from everything section 3 makes possible) and how it gets priced once quoted.
  - **4.1 Opening the Market** ([pages/opening-over-under-market.html](pages/opening-over-under-market.html)) — a Market Type dropdown toggles between Over/Under (sweeping lines 0.5–4) and Handicap (sweeping lines -2 to 2, both in quarter-point steps), reusing the same sweep-and-pick-the-closest-to-50/50 mechanic either way (see [ADR 0003](docs/adr/0003-extend-opening-market-with-market-type-toggle.md)). The exercise: pick the 1, 2, or 3 line(s) (a "Number of Candidate Main Lines" dropdown) whose two fair probabilities are closest to 50/50 — this course's simplified rule for a candidate main line, not a claim about how real bookmakers always open a market.
  - **4.2 Adding a Pricing Spread** ([pages/adding-a-spread.html](pages/adding-a-spread.html)) — same Market Type toggle as 4.1; takes a fixed main line (Over/Under 2.75, or Handicap Home -0.25 — the candidate main line for this dataset) and asks the user to fill in a second row: a 0.10 pricing spread applied to the fair Malay odds (half off each side), from which HK, Euro, and the resulting (now merely *implied*, overround-inclusive) Probability are derived. The spread only ever shifts the price, never the line.
- **5. From Historical Probability to Poisson** ([pages/poisson-distribution.html](pages/poisson-distribution.html)) — recaps the historical/empirical method from section 2, shows its small-sample limitation (e.g. 0% for a goal count that just never happened in 10 matches), introduces the Poisson PMF `P(X=k) = e^-λ × λ^k / k!` with λ = expected goal count / mean scoring rate (named to avoid confusion with football's own "Expected Goals (xG)" metric), states its two simplifying assumptions (a fixed scoring rate, and Home/Away goals modeled independently), compares historical vs Poisson probabilities for Home goals side by side, and shows that Poisson only changes how the Home/Away goal probabilities are estimated — the matrix/market/odds pipeline from sections 2&ndash;4 is unchanged. All numbers are computed live from `MATCH_HISTORY`, not hardcoded.
...
- `betTeamProbabilityFromStats(stats, point, key)` / `getCategories` / `pivotForDisplay` / `getMiddleActionLabel(point, integerLabel)` / `getSplitSettlementWords(point)` / `getMiddleLabel(point, integerLabel, sideLabels)` — the category-list/label/Selection-adapter helpers shared by 3.5 and 3.6. A quarter point's pivot settles the two bettable sides *oppositely* (one's a Half Loss, the other's a Half Win), so `getMiddleLabel` names both (e.g. "Over: Half Loss / Under: Half Win") for display; `getMiddleActionLabel` stays a single neutral phrase ("Split Settlement") for the paint button, which doesn't belong to either side.
...
- `isUndefinedOdds` / `toEuro` / `toHK` / `toMalay` / `malayToHK` — the odds conversion chain (Fair Probability ⇄ Euro (Decimal) Odds ⇄ HK Odds ⇄ Malay Odds), including the sign-based inverse used by 4.2's spread. `isUndefinedOdds` is true only for an exactly-zero probability (e.g. Correct Score's AOS in this dataset) - a small non-zero probability still has real, if very large, odds.
...
- **Formula-hover tooltips**: an empty input shows its `data-formula` on hover *or keyboard focus* (to the right of the cell), toggled globally by "Formula Hover: On/Off".
- **Cross-cell highlighting**: `data-cell` / `data-refs` let a formula's hover or keyboard focus also highlight (or, on exercises 4/5, fade) the source cell(s) it depends on.
- **Answer checking**: `markInput(input, expected, tolerance, decimals)` marks a cell correct/incorrect within a tolerance. Both arguments are required at every call site (no shared default) so a field's precision policy stays explicit and can't drift from what's displayed: raw probabilities, Settlement-Adjusted Fair Probability, and odds are all 2 decimals / 0.005 tolerance, and integer counts (2.1's goal frequency) are exact (0 tolerance).
...
- **Category picker**: `createCategoryPicker({containerId, categories, categoryClass, labels})` builds the free-selection button group (`getActive`/`setActive`/`reset`/`rebuild`) that drives which category `wireClicks` paints with - shared by the same five exercises, so a student can switch categories and repaint any cell at any time instead of being forced through a one-way click sequence. Over/Under and HDP call `rebuild` on every Line change, since their category count/labels depend on the selected line's point type.
- **Dim-complement hover**: `attachDimComplement(inputId, key, getStats, getCategories)` fades the *other* categories' matrix cells while hovering (or focusing) a probability formula - shared by the same five exercises.
- **Simple odds table**: `buildSimpleOddsTable` / `checkSimpleOddsTable` / `fillSimpleOddsTable` (plus `resetSimpleOddsTable` for the "given" case) build/check/fill a plain Fair Probability (-> Euro Odds (-> HK Odds -> Malay Odds)) table with no push/half-win exclusion - shared by 3.2, 3.3, and 3.4. `euroMode` (default `"input"`) controls Euro Odds: `"input"` makes it a graded field (3.2 Odd/Even); `"given"` makes it a derived `.given-value` cell revealed (not graded) once Probability checks out, used by 3.3's 1X2 table and 3.4 Total Goal - odds conversion isn't either exercise's main focus, but it's still shown, matching Double Chance's own Euro Odds column right below 1X2's table; `"omit"` would drop it entirely (not currently used by any page, but available). `includeHkMalay` (default `true`, only meaningful with `euroMode: "input"`) drops just the HK/Malay columns; only 3.2 Odd/Even uses the default, since 3.1, 3.3, and 3.4 only ever show Euro Odds. Over/Under and HDP (3.5, 3.6) still build their own push-aware Fair-Probability + Selection table, since that logic is more involved and only duplicated twice.
