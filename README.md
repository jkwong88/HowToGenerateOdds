...
- **3. Odds for Different Market Types** ([pages/market-types-overview.html](pages/market-types-overview.html)) — the given matrix every market in this section starts from, settlement logic (which scorelines make a bet win, and which don't settle cleanly), and the shared odds conversion chain.
  ...
  - **3.3 1X2 & Double Chance** ([pages/1x2-double-chance-market.html](pages/1x2-double-chance-market.html)) — Part 1 (1X2): paint every cell Home/Draw/Away, then Check verifies the grid and unlocks a 1X2 Fair Odds table (Probability → Euro Odds only - no HK/Malay for this market). Passing that table's Check unlocks Part 2 (Double Chance): fill in the Probability for 1X/12/X2 by summing the matching pair of 1X2 probabilities; Euro Odds is a pure derived value that reveals once Probability checks out (see [ADR 0001](docs/adr/0001-merge-1x2-and-double-chance.md)).
  - **3.4 Total Goals** ([pages/total-goal-market.html](pages/total-goal-market.html)) — the matrix cells become click targets across four fixed buckets from `bookie_example.png` (0–1 / 2–3 / 4–6 / 7+), then Check verifies the whole grid and unlocks a Total Goals Fair Odds table (Probability → Euro Odds only - no HK/Malay for this market).
  - **3.5 Over / Under** ([pages/over-under-market.html](pages/over-under-market.html)) — a Point dropdown (2, 2.25, 2.5, 2.75) changes the classification: a whole-number point has 3 categories (Under/Push/Over, a real refundable push), a `.25`/`.75` point also has 3 (Under/Split Settlement/Over, per the reference slides' EV derivation - the pivot scoreline is a Half Loss for whichever side sits above it and a Half Win for the other), and a `.5` point has only 2 (Under/Over, no push possible). The Selection conversion table only ever lists Under/Over, since a push or a split settlement isn't itself bettable.
  - **3.6 HDP** ([pages/hdp-market.html](pages/hdp-market.html)) — a Line dropdown (-1 to +1 in quarter-point steps, standard Asian Handicap sign) reuses Over/Under's exact point-type math, just classifying Home − Away instead of Home + Away (see [ADR 0002](docs/adr/0002-generalize-point-type-math-for-hdp.md)). Paint every cell Away Covers / Push-or-half / Home Covers, then Check unlocks an HDP Fair Odds table.
...
- `isUndefinedOdds` / `toEuro` / `toHK` / `toMalay` / `malayToHK` — the odds conversion chain (Fair Probability ⇄ Euro Odds ⇄ HK Odds ⇄ Malay Odds), including the sign-based inverse used by 4.2's spread.
